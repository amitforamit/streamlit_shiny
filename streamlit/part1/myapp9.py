...
def run_query(query):
    print("runnning query")
    data = pd.read_sql(query, conn)
    data['Date'] = pd.to_datetime(data['time']).dt.date
    #df_raw = pd.read_csv("DATA_NEW.csv")
    #df_raw['Date']= pd.to_datetime(df_raw['Date'])
    # chenaged "rejects" to "Reject Reason"
...
if not option4:
    st.write("Select a Machine. Machine should not be blank.")
else:
